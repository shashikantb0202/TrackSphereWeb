...
export class SidebarComponent {
  ...
  ngOnInit() {
    ...
    if (permission != null && permission.length > 0) {
      ...
      this.userPermissionService
        .getUserPermissionsGroup(this.loggedInUser?.id!)
        .subscribe((data) => {
  ...
  mapPermissionsToMenuStructure(permissions: any[]): any[] {
    const groupedModules = permissions.reduce((acc: any, curr: any) => {
      ...
      const hasModuleViewPermission =
        !curr.subModuleId && curr.permissions?.View;
  ...
  autoSetActiveItem() {
    ...
    this.sidebarMenu.forEach(
      (module: { subModules: any[]; moduleName: string }) => {
  ...
  toggleSubMenu(moduleName: string): void {
